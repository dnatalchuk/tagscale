	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				if err := costService.CollectCostData(ctx, start, end, time.Duration(cfg.AWSRequestTimeout)*time.Second); err != nil {
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				start := end.AddDate(0, 0, -30)
				if _, err := analysisService.RunAnalysis(5, start, end, true); err != nil {
